 ...
 */
public class GenericServo extends ComponentBase implements ServoBase {

    public enum Orientation {
        LEFT, RIGHT
    }

    public static final float PWM_MIN = 450; //900; // in micro seconds
    public static final float PWM_NEUTRAL = 1074; //1500; // in micro seconds
    public static final float PWM_MAX = 2048; //2100; // in micro seconds
...
//      pwmDuration = calculatePwmDuration(position);
        pwm = calculatePwm(position);
        servoDriver.setServoPulseWidth(pwm);
    }
    ...
    int calculatePwm(float position) {
    	return isReverse ? ((int) (-0.058 * Math.pow(position, 2) + 2048)) : ((int) (0.058 * Math.pow(position, 2) + 450));
    }
    ...
    private float validateEndPoint(float endPoint, String propertyName) {
        if (endPoint < ServoBase.END_POINT_MIN || endPoint > ServoBase.END_POINT_MAX) {
            throw new ValidationException("Value of property [" + propertyName + "] must be between " + ServoBase.END_POINT_MIN + " and " + ServoBase.END_POINT_MAX + " but is [" + endPoint + "]");
        }
        return endPoint;
    }

    private float validateSubtrim(float subtrim, String propertyName) {
        if (subtrim < ServoBase.SUBTRIM_MAX_LEFT || subtrim > ServoBase.SUBTRIM_MAX_RIGHT) {
            throw new ValidationException("Value of property [" + propertyName + "] must be between " + ServoBase.SUBTRIM_MAX_LEFT + " and +" + ServoBase.SUBTRIM_MAX_RIGHT + " but is [" + subtrim + "]");
        }
        return subtrim;
    }

    private float validatePosition(float position) {
        if (position < ServoBase.POS_MAX_LEFT || position > ServoBase.POS_MAX_RIGHT) {
            throw new ValidationException("Position [" + position + "] must be between " + ServoBase.POS_MAX_LEFT + "(%) and +" + ServoBase.POS_MAX_RIGHT + "(%) but is [" + position + "]");
        }
        return position;
    }
}
